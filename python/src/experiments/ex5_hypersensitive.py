"""
Legacy 2014 Example 3.1 / archive Example 5: hypersensitive optimal control.
"""
import csv
import json
import time
...
def build_hypersensitive_problem(gamma=1.0e6, u_min=-1.0, u_max=3.0):
    x0 = np.array([1.0])
    T = 25.0
    ...
    def hamiltonian_grad_fn(x, p, t):
        y = x[0]
        lam = p[0]
        grad_p = np.array([-(y**3) - 0.5 * lam])
        grad_x = np.array([-3.0 * lam * (y**2) + 2.0 * y])
    ...
    problem_data = {
        ...
        "u_max": float(u_max),
        "legacy_example": "3.1",
        "problem_name": "hyper-sensitive optimal control",
    }
    return prob, problem_data


def run_hypersensitive_solver(
    n_init=30,
    tol_time=5e-2,
    tol_PA=1e-2,
    tol_delta=1e-2,
    ...
    prob, problem_data = build_hypersensitive_problem(gamma=gamma, u_min=u_min, u_max=u_max)
    ...
    result["legacy_reference_label"] = "2014 reference manuscript Example 3.1"
    return result, prob
...
def _compute_controls(prob, bundle, X, P, t_nodes):
    ...
    for i, t_i in enumerate(t_nodes):
        _, u_star = compute_H(prob, P[i], X[i], t_i, bundle.controls, restricted=True, use_oracle=True)
        if u_star is None:
            raise RuntimeError(f"No admissible control found at hypersensitive node {i}.")
...
def _legacy_reference_rows():
    ...
    rows = [
        {"quantity": "legacy_reference_source", "value": "2014 manuscript Example 3.1 executable baseline"},
    ]
...
def _input_parameter_rows(result):
    ...
    return [
        ...
        {"parameter": "x0", "value": str(float(pdata["x0"][0]))},
        {"parameter": "T", "value": _format_value(pdata["T"])},
        {"parameter": "gamma", "value": _format_value(pdata["gamma"])},
        ...
        {"parameter": "use_oracle_bootstrap", "value": _format_value(settings["use_oracle_bootstrap"])},
        {"parameter": "use_oracle_PA", "value": _format_value(settings["use_oracle_PA"])},
        {
            "parameter": "use_explicit_hamiltonian_gradients",
            "value": _format_value(settings["use_explicit_hamiltonian_gradients"]),
        },
...
def _plot_iteration_state_costate(entry, out_path):
    ...
    axes[0].set_title("State trajectory with bundle support points")
    axes[0].grid(True)
    axes[0].legend()
...
def _plot_error_indicator_history(log, out_path):
    ...
    plt.semilogy(iterations, eta_time, marker="o", label=r"$\eta_{\mathrm{time}}$")
...
def _write_iteration_artifacts(log, iteration_root):
    for entry in log:
        if "X_iter" not in entry or "P_iter" not in entry or "U_iter" not in entry:
            raise ValueError("Deep report export requires store_iterates=True in the hypersensitive solver run.")
...
def _generate_ex31_report_tex(out_dir, summary, input_rows, reference_rows, outer_rows, log):
    intro_status = (
    ...
    for entry in log:
        ...
        if entry.get("action") == "add_plane" and pa_plan.get("target_count", 0) > 0:
            ...
            pa_tables = dedent(
                f"""
                {_latex_table_from_pairs(f"PA enrichment summary at iteration {iter_idx}.", f"tab:ex31-pa-summary-{iter_idx}", summary_rows)}

                {_latex_longtable_from_rows(
                    f"Selected PA enrichment candidates at iteration {iter_idx}.",
                    f"tab:ex31-pa-candidates-{iter_idx}",
    ...
    report = dedent(
        ...
        \\title{{Standalone Deep Report for Legacy 2014 Example 3.1}}
        ...
        \\section{{Legacy 2014 Example 3.1: Hyper-Sensitive Optimal Control}}
        This standalone report documents the current executable hypersensitive example derived from the 2014 reference manuscript. The purpose is diagnostic and comparative: every adaptive outer iterate is exposed with the same plot family and the same summary fields used in the Example 1 deep report, so the solver trajectory can be inspected and compared carefully against the legacy baseline.
        ...
        {_latex_table_from_pairs("Input parameters used for the hypersensitive run.", "tab:ex31-input-parameters", input_pairs)}

        \\subsection{{Reference values}}
        {_latex_table_from_pairs("Legacy reference values currently available for Example 3.1.", "tab:ex31-reference-values", reference_pairs)}

        \\subsection{{Outer-loop summary}}
        {_latex_longtable_from_rows("Adaptive outer-loop iterations of the proposed solver on legacy Example 3.1.", "tab:ex31-outer-loop", outer_display_fields, outer_display_rows)}

        \\subsection{{Final reported numbers}}
        {_latex_table_from_pairs("Final numbers reported by the proposed solver on the hypersensitive example.", "tab:ex31-final-summary", final_pairs)}

        \\subsection{{Error-estimate history}}
        The figure below shows the progress of the time-discretization, plane-approximation, and smoothing indicators against the outer iteration count.
...
def export_ex31_deep_report_artifacts(result, prob, out_dir):
    ...
    report_path = _generate_ex31_report_tex(out_dir, summary, input_rows, reference_rows, outer_rows, result["log"])
    return {
        "summary": summary,
...
def summarize_hypersensitive_results(result):
    last = result["log"][-1]
    print("\nLegacy 2014 Example 3.1 (Hypersensitive optimal control)")
...
def run_example():
    result, _ = run_hypersensitive_solver(
        use_oracle_bootstrap=False,
        use_oracle_PA=True,
        use_explicit_hamiltonian_gradients=False,
        store_iterates=False,
